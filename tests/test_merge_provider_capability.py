...
class MergeProviderCapabilityTests(unittest.TestCase):
    def test_observed_github_profile_is_unproven(self) -> None:
        ...
        self.assertIsNone(result["selected_operation"])
        by_id = {item["id"]: item for item in result["operation_assessments"]}
        self.assertIn("base_precondition=NONE requires EXACT_BASE", by_id["REST_PULL_MERGE"]["reasons"])
        self.assertIn("base_precondition=FAST_FORWARD_ONLY requires EXACT_BASE", by_id["REST_GIT_REF_UPDATE"]["reasons"])
    ...
    def test_head_only_operation_never_substitutes_for_base_cas(self) -> None:
        ...
        self.assertEqual(result["decision"], "UNPROVEN")
        self.assertIn("base_precondition=NONE requires EXACT_BASE", result["reasons"])
    ...
    def test_fast_forward_is_not_claimed_as_exact_base_cas(self) -> None:
        ...
        result = capability.assess(evidence)
        self.assertEqual(result["decision"], "UNPROVEN")
        self.assertIn("base_precondition=FAST_FORWARD_ONLY requires EXACT_BASE", result["reasons"])
    ...
    def test_latest_base_queue_changes_the_certificate_contract(self) -> None:
        ...
        result = capability.assess(evidence)
        self.assertEqual(result["decision"], "UNPROVEN")
        self.assertIn("base_precondition=LATEST_BASE requires EXACT_BASE", result["reasons"])
